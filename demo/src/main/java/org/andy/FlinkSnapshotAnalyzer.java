...
public class FlinkSnapshotAnalyzer {
  ...
  public static String savepointDirectory = "D:\\tmp\\flink\\savepoint-f680f0-292195460afd";
...
//    orderBySize();
  ...
  private static void printDetail() throws IOException {
    ...
    for (OperatorState operatorState : deserializedMetadata.getOperatorStates()) {
      operatorState.getStates().forEach(o -> {
        log.info("state: {}", o.toString());
        o.getRawOperatorState().forEach(r ->{
                  log.info(r.getStateNameToPartitionOffsets().toString());
                }
        );
      });
  ...
  private static void orderBySize() throws IOException {
