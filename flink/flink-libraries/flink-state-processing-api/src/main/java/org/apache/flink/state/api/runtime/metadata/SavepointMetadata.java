...
public class SavepointMetadata {
    ...
    public OperatorState getOperatorStateByHexString(String hexString) throws IOException {
        OperatorID operatorID = new OperatorID(StringUtils.hexStringToByte(hexString));

        OperatorStateSpec operatorState = operatorStateIndex.get(operatorID);
        if (operatorState == null || operatorState.isNewStateTransformation()) {
            throw new IOException(
                    "Savepoint does not contain state with operator uid " + hexString);
        }
